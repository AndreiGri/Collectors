...
string Declination(int a, string text1, string text2, string text3) {                   // Метод для правильного склонения единиц измерения
    string text;
    if (a % 10 == 0 || a % 10 >= 5 || (a / 10) % 10 == 1) {                             // Условиями присваиваем переменной text необхоимое склонение слова
        text = text1;
    }
    else {
        if (a % 10 == 1) {
...
int main()
{
    ...
    string text1 = " рублей!";                                                                       // Переменные с возможными склонениями
    string text2 = " рубль!";
    string text3 = " рубля!";
    ...
    while (duty > 0) {                                                                               // Цикл для запроса суммы списания и вычисления остатка по кредиту
        ...
        cout << " " << name << " Ваш долг перед банком составляет: " << duty << text << endl;        // Вывод сообщения о долге клиента
        cout << " " << endl;
    ...
    if (duty <= 0) {                                                                                 // Вывод сообщения о погашении кредита с отрицательным балансом кредита
        string text = Declination(duty * (-1), text1, text2, text3);                                 // Локальная переменная для записи правильного склонения
                                                                                                     // вызываемым методом Declination
        cout << " Долг погашен, долг за банком составляет: " << duty * (-1) << text << endl;         // Выводим сообщение о погашении долга и задолженности банка
    }
